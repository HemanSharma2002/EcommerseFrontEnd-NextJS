...
import { initiateOnlinePayment, updateRazorpayPaymentInformation } from '@/app/backendApiCalls/api'
import { Button } from '@mui/material'
import Link from 'next/link'
import React from 'react'
import useRazorpay, { RazorpayOptions } from 'react-razorpay'

type Props = { order: Order,loadPage:Function }

export default function OrderCard({ order,loadPage }: Props) {
            ...
            <div className=' border-2 border-blue-950 shadow-md w-full h-full text-blue-950 rounded-md'>
                <div className=' flex flex-row gap-3'>
                    <div className=' flex flex-col gap-2 p-8  w-52'>
                        <p>Order Id </p>
                        <p># {order.id}</p>
                        <Link href={`/user/order/${order.id}`} className='pt-8'>
